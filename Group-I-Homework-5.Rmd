...
$$
So we have to calculate the partial derivative of the log likelihood respect to $\mu$ and set it equal 0
$$
...
```
These two models differ only in the use of `weight^2`. So ANOVA will test whether or not including this leads significant improvement. Adding this term spawned non-significant result (p=0.259). Thus adding `weight^2` didn't lead better result. In other words, second model didn't explain data well.
